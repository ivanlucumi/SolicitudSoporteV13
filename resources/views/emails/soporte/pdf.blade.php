<!DOCTYPE html>
<!--
This is a starter template page. Use this page to start your new project from
scratch. This page gets rid of all links and provides the needed markup only.
-->
<html>
<head>
  <meta charset="utf-8">
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
  <meta name="author" content="">
  <title>@yield('title')<?php $time = time(); echo date("d-m-Y", $time);?></title>
  <?php
    header("Content-Transfer-Encoding: binary");
    header("Content-Type: image/png");
  ?>
  <link href="css/app.css" rel="stylesheet">
  <style type="text/css">
  	body{
        font-size: 10;
  	}
      @font-face {
  font-family: 'Open Sans';
  font-style: normal;
  size: 10px;
  font-weight: normal;
  src: url(http://themes.googleusercontent.com/static/fonts/opensans/v8/cJZKeOuBrn4kERxqtaUH3aCWcynf_cDxXwCLxiixG1c.ttf) format('truetype');
}
  </style>

  <style type="text/css">
    .organizar{
      border: 1px solid black;border-collapse: collapse;
    }
  </style>



 <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
 <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
</head>

<body style="background: #fff;" >
  <main role="main" class="container">
    <div class="row">
            <table id="table9" class="table table-bordered table-striped  table-condensed table-hover" >
               <tbody class="shadow" >
                 <tr>

                       <th>
                        <p style="text-align: center; font-size: 10px; font-weight: bold;">
                            <br>Consejo Superior de la Judicatura<br>
                            Direcci&oacute;n Ejecutiva Seccional de Administraci&oacute;n Judicial<br>
                            Cali - Valle del Cauca
                          </p>
                       </th>
                       <th>
                        <img src="data:image/png;base64," alt="">
                       </th>

                 </tr>

               </tbody>

                       <tbody data-id="" class="buscar">
                        <tr>
                            <th colspan="2">
                                <center>FORMATO REPORTE DE DIAGNOSTICO ON SITE</center>
                            </th>
                         </tr>


                         <tr>
                                    <th style="width: 30%">
                                    <p style="text-align: left;">
                                       N&uacute;mero de Caso
                                    </p>

                                    </th>
                                    <th style="width: 60%">
                                     <input id="num_caso" class="form-control @error('num_caso') is-invalid @enderror" placeholder="Ingrese N&uacute;mero de Caso" autocomplete="off" type="text" name="num_caso" value="{{ old('num_caso', $soporte->num_caso) }}">
@error('num_caso')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror

                                    </th>
                         </tr>

                         <tr>
                            {{$soporte->fecha_solicitud}}
                          <th colspan="2">
                                            <label for=""style="width: 30%;text-align: center;">* Fecha Solicitud:</label>
                                            <label for=""style="width: 30%;text-align: center;">* Hora Solicitud:</label>
                                            <label for=""style="width: 30%;text-align: center;">* Medio Solicitud:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->fecha_solicitud}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->hora_solicitud}}</label>
                                            <label style="width: 30%;"><input type="radio" style="float: left;" id="cbox3" value="{{$soporte->medio_solicitud}}" name="medio_solicitud" checked  required> {{$soporte->medio_solicitud}}</label>

                            </th>

                         </tr>

                         <tr>
                            <th colspan="2">
                              <p class=""><h5><center>Despacho:</center></h5></p>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                            <label for=""style="width: 30%;">* SECCIONAL:</label>
                                            <label for=""style="width: 30%;">* DESPACHO:</label>
                                            <label for=""style="width: 30%;">* DIRECCI&Oacute;N:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->seccional}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->despacho}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->direccion}}</label>

                            </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                              <p class=""><h5><center>Datos de Usuario:</center></h5></p>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                            <label for=""style="width: 30%;">* IDENTIFICACION:</label>
                                            <label for=""style="width: 30%;">* NOMBRES:</label>
                                            <label for=""style="width: 30%;">* APELLIDOS:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->cedula}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->nombre}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->apellido}}</label>


                                            <label for=""style="width: 30%;">* CIUDAD:</label>
                                            <label for=""style="width: 30%;">* CARGO:</label>
                                            <label for=""style="width: 30%;">* TEL&Eacute;FONO:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->ciudad}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->cargo}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->telefono}}</label>

                            </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                            <p class=""><h5><center>Falla Reportada</center></h5></p>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                            <textarea id="falla_reportada" style="height:6em" class="form-control @error('falla_reportada') is-invalid @enderror" placeholder="Registre Falla Reportada" min="1" autocomplete="off" name="falla_reportada">{{ old('falla_reportada', $soporte->falla_reportada) }}</textarea>
@error('falla_reportada')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror
                           </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                            <p class=""><h5><center>Ingeniero Asignado</center></h5></p>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                            <label for=""style="width: 45%;">* Fecha Atenci&oacute;n:</label>
                                            <label for=""style="width: 45%;">* Hora Atenci&oacute;n:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->fecha_atencion}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->hora_atencion}}</label>

                                            <label for=""style="width: 45%;">* Nombre:</label>
                                            <label for=""style="width: 45%;">* Tipo de Servicio:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->nombre_tecnico}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->tipo_servicio}}</label>

                            </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                                <p class=""><h5><center>Datos Equipo Afectado</center></h5></p>
                            </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                                            <label for=""style="width: 30%;">* Placa:</label>
                                            <label for=""style="width: 30%;">* Serial Equipo:</label>
                                            <label for=""style="width: 30%;">* Marca Equipo:</label>

                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->placa}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->serial_equipo}}</label>
                                            <label for=""style="width: 30%;text-align: center;">{{$soporte->marca_equipo}}</label>

                                            <label for=""style="width: 23%;">* Modelo Equipo:</label>
                                            <label for=""style="width: 23%;">* Sistema Operativo:</label>
                                            <label for=""style="width: 23%;">* Antivirus:</label>
                                            <label for=""style="width: 23%;">* Versi&oacute;n Antivirus:</label>

                                            <label for=""style="width: 23%;text-align: center;">{{$soporte->modelo_equipo}}</label>
                                            <label for=""style="width: 23%;text-align: center;">{{$soporte->sistema_operativo}}</label>
                                            <label for=""style="width: 23%;text-align: center;">{{$soporte->antivirus}}</label>
                                            <label for=""style="width: 23%;text-align: center;">{{$soporte->ver_antivirus}}</label>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                <p class=""><h5><center>Elementos de Soporte</center></h5></p>
                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                              <label for=""style="width: 40%;text-align: center;">* Usa Elementos:</label>
                                              <label><input type="radio"  id="cbox3" value="{{$soporte->elementos_de_soporte}}" name="elementos_de_soporte" checked  required> {{$soporte->elementos_de_soporte}}</label>

                              </th>

                           </tr>
                           <tr>
                            <th colspan="2">
                                <hr>
                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                <label for=""style="width: 100%;text-align: left;">* Elemento Usado:</label><br>
                                    <label><input type="radio" style="width: 33%;" id="cbox3" value="{{$soporte->elementos_de_soporte}}" name="tipo_elemento_de_soporte"  required> {{$soporte->elementos_de_soporte}}</label>
                                    <label for=""style="width: 23%;">{{$soporte->elemeto_soporte}}</label>

                              </th>

                           </tr>

                          <tr>
                            <th colspan="2">
                                <p class=""><h5><center>Elementos de Soporte</center></h5></p>
                            </th>
                          </tr>

                          <tr>
                            <th colspan="2">
                                            <label for=""style="width: 30%;">* Placa:</label>
                                            <label for=""style="width: 30%;">* Serial Equipo:</label>
                                            <label for=""style="width: 30%;">* Marca Equipo:</label>
                                            <input type="text" name="placa_soporte" style="width: 30%;" class='form' value="{{$soporte->placa_soporte}}" >
                                            <input type="text" name="serial_equipo_soporte" style="width: 30%;" class='form' value="{{$soporte->serial_equipo_soporte}}">
                                            <input type="text" name="marca_equipo_soporte" style="width: 30%;" class='form' value="{{$soporte->marca_equipo_soporte}}">

                                            <label for=""style="width: 23%;">* Modelo Equipo:</label>
                                            <label for=""style="width: 23%;">* Sistema Operativo:</label>
                                            <label for=""style="width: 23%;">* Memoria Ram:</label>
                                            <label for=""style="width: 23%;">* Disco Duro:</label>
                                            <input type="text" name="modelo_equipo_soporte" style="width: 23%;" class='form' value="{{$soporte->modelo_equipo_soporte}}">
                                            <input type="text" name="sistema_operativo_soporte" style="width: 23%;" class='form' value="{{$soporte->sistema_operativo_soporte}}">
                                            <input type="text" name="memoria_soporte" style="width: 23%;" class='form' value="{{$soporte->memoria_soporte}}">
                                            <input type="text" name="disco_soporte" style="width: 23%;" class='form' value="{{$soporte->disco_soporte}}">

                                            <label for=""style="width: 30%;">* Procesador:</label>
                                            <label for=""style="width: 30%;">* Tipo Equipo:</label>
                                            <label for=""style="width: 30%;">* Nombre Equipo:</label>
                                            <input type="text" name="procesador_soporte" style="width: 30%;" class='form' value="{{$soporte->procesador_soporte}}">
                                            <input type="text" name="tipo_equipo_soporte" style="width: 30%;" class='form' value="{{$soporte->tipo_equipo_soporte}}">
                                            <input type="text" name="nombre_equipo_soporte" style="width: 30%;" class='form' value="{{$soporte->nombre_equipo_soporte}}">

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                            <p class=""><h5><center>Descripci&oacute;n del Servicio</center></h5></p>
                            </th>
                          </tr>
                          <tr>
                                <th colspan="2">
                                <label for="fecha">* Diagn&oacute;stico:</label><br>
                                <textarea id="diagnostico" style="width: 95%;font-size:10;" class="form-control @error('diagnostico') is-invalid @enderror" placeholder="Registre Diagn&oacute;stico" min="1" autocomplete="off" name="diagnostico">{{ old('diagnostico', $soporte->diagnostico) }}</textarea>
@error('diagnostico')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror<br>

                                <label for="fecha">* Soluci&oacute;n:</label><br>
                                <textarea id="solucion" style="width: 95%;font-size:10;" class="form-control @error('solucion') is-invalid @enderror" placeholder="Registre Soluci&oacute;n" min="1" autocomplete="off" name="solucion">{{ old('solucion', $soporte->solucion) }}</textarea>
@error('solucion')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror<br>
                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                            <p class=""><h5><center>Observaci&oacute;n Cliente</center></h5></p>

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                    <input id="observacion_cliente" style="width: 95%;font-size:10;" class="form-control @error('observacion_cliente') is-invalid @enderror" placeholder="Observaciones" autocomplete="off" type="text" name="observacion_cliente" value="{{ old('observacion_cliente', $soporte->observacion_cliente) }}">
@error('observacion_cliente')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror

                            </th>
                          </tr>
                          <tr>
                            <th colspan="2">
                                <label for=""style="width: 70%; text-align: left;">* Disposici&oacute;n para atenci&oacute;n de servicio del ingeniero:</label>
                                 <label style="width: 20%;">{{$soporte->disposicion}} </label><br>

                                 <label for=""style="width: 70%; text-align: left;">* Conocimiento t&eacute;cnico del ingeniero :</label>
                                 <label style="width: 20%;">{{$soporte->conocimiento_tec}} </label><br>

                                 <label for=""style="width: 70%; text-align: left;">* Tiempo de atenci&oacute;n y soluci&oacute;n:</label>
                                 <label style="width: 20%;">{{$soporte->tiempo_aten}} </label><br>

                                 <label for=""style="width: 70%; text-align: left;">* Informaci&oacute;n del avance del caso :</label>
                                 <label style="width: 20%;">{{$soporte->avance_caso}} </label><br>


                            </th>
                       </tr>
                       <tr>
                        <th colspan="2">
                        <p class=""><h5><center>Firma Funcionario</center></h5></p><br>
                        <img height="120em" src="{{$soporte->firma}}" alt="">

                        </th>
                      </tr>
             </tbody>
            </table>



 </main>





<!-- REQUIRED JS SCRIPTS   onClick="this.disabled=true"-->

<!-- jQuery 3 -->
<script src="adminlte/bower_components/bootstrap/dist/js/bootstrap.min.js"></script>
<script src="adminlte/dist/js/adminlte.min.js"></script>


</body>

</html>


