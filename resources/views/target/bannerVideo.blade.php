...
<style>
  #banner {
    ...
    max-width: 100%; /* Opcional: Ancho máximo del contenedor */
    overflow: hidden; /* Evita que el video se desborde del contenedor */
  }

  #video {
    width: 100%; /* Opcional: Ancho completo del video */
      ...
      <center>
       <div id="banner">
          <video id="video" autoplay muted >
            <source src="/video/Video Invitacion TCA.mp4" type="video/mp4">
            <source src="/video/TAVfinalDispositivo.mp4" type="video/mp4">
...
<!--script>
  document.addEventListener('DOMContentLoaded', function() {
    const videos = [
      '/video/Video Invitacion TCA.mp4',
      '/video/TAVfinalDispositivo.mp4'
    ]; // Lista de videos, puedes añadir más
    ...
    function playNextVideo() {
      ...
      video.play();
      //video.muted = false;
      currentVideo = (currentVideo + 1) % videos.length; // Avanza al siguiente video circularmente
    }
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function playNextVideo() {
      video.src = videos[currentVideo];
      video.play().then(() => {
        //video.muted = false; // Habilitar sonido después de la reproducción automática
      }).catch((error) => {
        
      });
      currentVideo = (currentVideo + 1) % videos.length; // Avanzar al siguiente video circularmente
    }
    ...
    function playNextVideo() {
      video.src = videos[currentVideo];
      video.play().then(() => {
        // Aquí podrías habilitar el sonido si el navegador lo permite
        // Por ejemplo, con un evento de clic o alguna otra interacción del usuario
        video.addEventListener('click', function enableSound() {
          video.muted = false;
          video.removeEventListener('click', enableSound); // Remueve el listener después de habilitar el sonido
        });
      }).catch((error) => {
