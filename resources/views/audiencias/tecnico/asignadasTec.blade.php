...
@section('title', 'Solicitud Audiencia Agendada')
@section('cabecera', ' Agendadas Por Mi ')

@section('content') 

<div class="" style="text-align: left">	
	<a href="{!! url('tecnico/solicitudes/agendadas')!!}" class="btn btn-warning btn-sm" >Audiencias </a>
</div>
	    ...
	    <thead class="shadow" style="background-color: #004182; color: #fff;">
            ...
            <tbody data-id="{!!$solicitud->id!!}" class="buscar " >
                <tr class="table-light">
                    <th><a href="{{ route('tecnico.corregir.audiencia', $solicitud->id) }}" class="btn btn-primary fa fa-pencil"> </a></th>
                    <th scope="row">{{$solicitud->email}}</th>									
                    <th >{{$solicitud->nombre_entidad}}</th>
                    <th scope="row">{{$solicitud->fecha_prgramada}}</th>
                    <th scope="row">{{$solicitud->fecha_asignacion}}</th>
