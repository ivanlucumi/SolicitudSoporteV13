    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus {
            ...
            background: #fff;
            box-shadow: 0 0 0 4px rgba(0, 63, 116, 0.15);
        }
        .field-hint {
            font-size: .78rem;
            color: var(--gris);
        ...
        .btn-consultar:hover  { transform: translateY(-2px); filter: brightness(1.08); box-shadow: 0 10px 28px rgba(0, 63, 116, 0.5); }
...
<header>
    <img src="{{ asset('/img/LogoRama.png') }}" alt="DISAJ Cali" onerror="this.style.display='none">
...
<footer>
    &copy; {{ date('Y') }} Todos los Derechos Reservados - SIRIS Cali
</footer>
...
<script>
    ...
    // ── Restricción: Solo dispositivos móviles ──
    (function() {
        const ua = navigator.userAgent || navigator.vendor || window.opera;

        // Detectar tabletas: iPad, Android tablets, Kindle, etc.
        const isTablet = /(ipad|tablet|(android(?!.*mobile))|(windows(?!.*phone)(.*touch))|kindle|playbook|silk|(puffin(?!.*(IP|AP|WP))))/i.test(ua);

        // Detectar escritorio: Windows, Mac, Linux que NO sean móvil ni tablet
        const isMobile = /android|webos|iphone|ipod|blackberry|windows phone|mobile/i.test(ua);

        const isDesktop = !isMobile && !isTablet;

        if (isDesktop || isTablet) {
            // Ocultar formulario y mostrar bloqueo
            document.querySelector('.card-wrap').innerHTML = `
                <div class="card" style="text-align:center; padding: 48px 28px;">
                    <div style="width:80px; height:80px; background:#fef2f2; border-radius:50%; display:flex; align-items:center; justify-content:center; margin: 0 auto 24px; border: 2px solid #fecaca;">
                        <i class="fa-solid fa-mobile-screen-button" style="font-size:2.2rem; color:#ef4444;"></i>
                    </div>
                    <h2 style="font-size:1.2rem; font-weight:700; color:#1a1a2e; margin-bottom:10px;">Acceso Solo Desde Celular</h2>
                    <p style="color:#6b7280; font-size:.9rem; line-height:1.6; margin-bottom:24px;">
                        La consulta y generación del carnet digital institucional está disponible<br>
                        <strong>únicamente desde dispositivos móviles (celulares)</strong>.<br><br>
                        Utiliza tu teléfono para continuar con el proceso.
                    </p>
                    <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:14px; display:flex; align-items:center; gap:10px; justify-content:center; color:#64748b; font-size:.82rem;">
                        <i class="fa-solid fa-shield-halved" style="color:#003f74;"></i>
                        Medida de seguridad institucional — DISAJ Cali
                    </div>
                </div>`;
            document.querySelector('.hero p').innerText = 'Acceso restringido a dispositivos móviles';
        }
    })();
</script>
</body>
