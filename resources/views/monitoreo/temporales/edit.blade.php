...
@section('cabecera', 'Editar Ingreso Temporal')

@section('content')
<div class="box box-warning">
    <div class="box-header with-border">
        <h3 class="box-title">Modificar Datos de Personal Temporal</h3>
    </div>
    <div class="box-body">
        <form action="{{ route('cooringreso.temporales.update', $temporal->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group @error('cedula') has-error @enderror">
                        <label>Cédula:</label>
                        <input type="number" name="cedula" class="form-control" value="{{ old('cedula', $temporal->cedula) }}" required>
                        @error('cedula') <span class="help-block">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group @error('nombre') has-error @enderror">
                        <label>Nombre Completo:</label>
                        <input type="text" name="nombre" class="form-control" value="{{ old('nombre', $temporal->nombre) }}" required>
                    ...
                    <div class="form-group @error('empresa') has-error @enderror">
                        ...
                        <input type="text" name="empresa" class="form-control" value="{{ old('empresa', $temporal->empresa) }}" required>
                ...
                <div class="col-md-6">
                    <div class="form-group @error('placa') has-error @enderror">
                        ...
                        <input type="text" name="placa" class="form-control" value="{{ old('placa', $temporal->placa) }}" required>
    ...
    @foreach(['CARRO' => 'CARRO', 'MOTO' => 'MOTO'] as $key => $value)
        <option value="{{ $key }}" @selected(old('tipo_vehiculo', old('tipo_vehiculo', $temporal->tipo_vehiculo)) == $key)>{{ $value }}</option>
    @endforeach
</select>
@error('tipo_vehiculo')
...
@enderror
    ...
    @foreach($ciudades as $key => $value)
        <option value="{{ $key }}" @selected(old('ciudad', old('ciudad', $temporal->juzgado)) == $key)>{{ $value }}</option>
    @endforeach
</select>
@error('ciudad')
    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group @error('fecha_inicio') has-error @enderror">
                        ...
                        <input type="date" name="fecha_inicio" class="form-control" value="{{ old('fecha_inicio', $temporal->fecha_inicio) }}" required>
                ...
                <div class="col-md-6">
                    <div class="form-group @error('fecha_fin') has-error @enderror">
                        ...
                        <input type="date" name="fecha_fin" class="form-control" value="{{ old('fecha_fin', $temporal->fecha_fin) }}" required>
