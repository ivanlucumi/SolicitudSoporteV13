...
@section('cabecera', 'Bienvenido al Panel de Registro de visitas')

@section('content') 
    
 <a href="{{ route('coordinador.agendamiento') }}" class="btn btn-primary">AGENDAR VISITA</a>
    ...
    <nav class="navbar navbar-light bg-light">
      <form action="{{ route('coordinador.ingreso') }}" method="POST">
    @csrf
	    ...
	    <thead style="background-color: #AFAFAF; color: #fff;">
			...
			<tbody data-id="{!!$ingreso->id!!}" class="buscar">
				<tr class="table-light">
					...
					<th scope="row">{{Carbon\Carbon::parse($ingreso->hora_ingreso)->format('h:m A')}}</th>									
