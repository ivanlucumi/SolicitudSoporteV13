...
<input type="hidden" id="siniestro_id" value="{{ isset($siniestro_edit) ? $siniestro_edit->id : '' }}">
...
<div style="max-width:1080px; margin:0 auto; padding-bottom:80px;">
    ...
    <div class="sin-card-body">
      <div class="row">
        <div class="col-sm-4 fgroup">
          ...
          <input type="date" id="fecha_siniestro" class="fcontrol" value="{{ isset($siniestro_edit) ? $siniestro_edit->fecha_siniestro->format('Y-m-d') : date('Y-m-d') }}">
        </div>
        <div class="col-sm-4 fgroup">
  ...
  <div class="sin-card">
    ...
    <div class="sin-card-body">
        ...
        <div class="col-sm-12 fgroup">
          ...
          <select id="sel-despacho" class="fcontrol select2" style="width: 100%;" onchange="onDespachoSeleccionado($(this).find(':selected'))">
            ...
            @foreach($despachos as $d)
              @php $selected = (isset($siniestro_edit) && $siniestro_edit->despacho_codigo === $d->codigoDespacho) ? 'selected' : ''; @endphp
              <option value="{{ $d->codigoDespacho }}" {{ $selected }}
      ...
      <div class="row" id="despacho-info" style="display:{{ isset($siniestro_edit) && $siniestro_edit->despacho_codigo ? 'flex' : 'none' }}; margin-top:4px;">
        <div class="col-sm-2 fgroup">
          <label class="flabel">Código</label>
          <input type="text" id="d-codigo" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_codigo ?? '' }}">
        </div>
        <div class="col-sm-5 fgroup">
          <label class="flabel">Nombre</label>
          <input type="text" id="d-nombre" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_nombre ?? '' }}">
        </div>
        <div class="col-sm-3 fgroup">
          <label class="flabel">Ciudad</label>
          <input type="text" id="d-ciudad" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_ciudad ?? '' }}">
        </div>
        <div class="col-sm-2 fgroup">
          <label class="flabel">Teléfono</label>
          <input type="text" id="d-telefono" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_telefono ?? '' }}">
        </div>
        <div class="col-sm-6 fgroup">
          <label class="flabel">Dirección</label>
          <input type="text" id="d-dir" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_direccion ?? '' }}">
        </div>
        <div class="col-sm-6 fgroup">
          <label class="flabel">Correo del Despacho</label>
          <input type="text" id="d-correo" class="fcontrol" readonly value="{{ $siniestro_edit->despacho_correo ?? '' }}">
        </div>
      </div>
    </div>
  </div>
          ...
          <div class="row">
            <div class="col-sm-6 fgroup">
              ...
              <div style="display:flex; gap:6px;">
                <input type="text" id="t-cedula" class="fcontrol" placeholder="Cédula" onkeypress="if(event.key==='Enter') buscarTitular()" value="{{ $siniestro_edit->titular_cedula ?? '' }}">
            ...
            <div class="col-sm-6 fgroup">
              <label class="flabel">Nombre</label>
              <input type="text" id="t-nombre" class="fcontrol" readonly placeholder="Autocompletado" value="{{ $siniestro_edit->titular_nombre ?? '' }}">
            </div>
          </div>
          <div class="row">
            <div class="col-sm-12 fgroup">
              <label class="flabel">Cargo</label>
              <input type="text" id="t-cargo" class="fcontrol" readonly placeholder="Autocompletado" value="{{ $siniestro_edit->titular_cargo ?? '' }}">
            </div>
          </div>
          <div class="row">
            <div class="col-sm-6 fgroup">
              <label class="flabel">Correo</label>
              <input type="email" id="t-correo" class="fcontrol" placeholder="Opcional" value="{{ $siniestro_edit->titular_correo ?? '' }}">
            </div>
            <div class="col-sm-6 fgroup">
              <label class="flabel">Teléfono</label>
              <input type="text" id="t-telefono" class="fcontrol" placeholder="Opcional" value="{{ $siniestro_edit->titular_telefono ?? '' }}">
            </div>
          </div>
        </div>
  ...
  <div class="sin-card" id="sec-finalizar">
    ...
    <div class="sin-card-body">
      <div class="row">
        <div class="col-sm-12 fgroup">
          ...
          <textarea id="observaciones_generales" class="fcontrol" rows="3"
            placeholder="Describa las circunstancias del siniestro, daños generales, acciones tomadas, recomendaciones...">{{ $siniestro_edit->observaciones_generales ?? '' }}</textarea>
        </div>
      </div>
      <div class="row" style="margin-top:15px;">
        <div class="col-sm-6 fgroup">
          <label class="flabel">URL de Fotos / Archivos <span style="font-weight:normal;color:#888;">(Si no se suben fotos)</span></label>
          <input type="url" id="url_fotos" class="fcontrol" placeholder="https://drive.google.com/..." value="{{ $siniestro_edit->url_fotos ?? '' }}">
        </div>
        <div class="col-sm-6 fgroup">
      ...
      <div style="text-align:center; padding-top:12px; display:flex; flex-direction:column; align-items:center; gap:10px;">
        <div style="display:flex; justify-content:center; gap:15px; flex-wrap:wrap;">
          <button type="button" class="btn btn-default" onclick="guardarBorradorYSalir()" style="border-radius:30px; font-weight:600; padding:12px 25px; box-shadow:0 4px 10px rgba(0,0,0,0.1);">
        ...
        <p style="margin-top:10px; font-size:.78rem; color:#888;">
          Al registrar y enviar, se generará el PDF y se enviará correo automáticamente al despacho.
        </p>
      </div>
    </div>
  </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if(canvas) {
        ...
        @if(isset($siniestro_edit) && $siniestro_edit->firma_empleado)
            signaturePad.fromDataURL('{{ $siniestro_edit->firma_empleado }}');
        @endif
    }

    let elementosCargados = @json(isset($siniestro_edit) ? $siniestro_edit->elementos : []);
    if (elementosCargados.length > 0) {
        elementosCargados.forEach(elem => {
            agregarElemento(); // Crea la UI base
            let idx = elementoCount;
            document.getElementById(`ei-${idx}`).value = elem.id;
            
            let elTipo = document.getElementById(`et-${idx}`);
            let tipo = (elem.tipo_elemento || '').trim();
            // Buscar si existe una opción que coincida (ignorando espacios extras)
            let match = Array.from(elTipo.options).find(o => (o.value || o.text).trim() === tipo);
            
            if (!match) {
                // Si no existe, lo agregamos y lo seleccionamos
                let newOpt = new Option(tipo, tipo, true, true);
                elTipo.add(newOpt);
                match = newOpt;
            }
            
            // Asignar el valor y notificar a Select2
            elTipo.value = match.value;
            if (typeof jQuery !== 'undefined') {
                jQuery(`#et-${idx}`).val(match.value).trigger('change');
            }

            if(document.getElementById(`ep-${idx}`)) document.getElementById(`ep-${idx}`).value = elem.placa || '';
            if(document.getElementById(`es-${idx}`)) document.getElementById(`es-${idx}`).value = elem.serial || '';
            if(document.getElementById(`ed-${idx}`)) document.getElementById(`ed-${idx}`).value = elem.descripcion_dano || '';
            if(document.getElementById(`eo-${idx}`)) document.getElementById(`eo-${idx}`).value = elem.observaciones || '';

            // Marcar como guardado (porque viene de la BD)
            elemGuardados[idx] = elem.id;
            const ew = document.getElementById(`ew-${idx}`);
            const eb = document.getElementById(`eb-${idx}`);
            ew.classList.remove('sin-guardar');
            ew.classList.add('guardado');
            eb.className = 'elem-badge badge-ok';
            eb.innerHTML = '<i class="fa fa-check"></i> Guardado';
            document.getElementById(`bs-${idx}`).innerHTML = '<i class="fa fa-save"></i> Guardar Elemento';

            // Cargar fotos
            if (elem.fotos && elem.fotos.length > 0) {
                elem.fotos.forEach(foto => {
                    let url = foto.url ? foto.url : '{{ asset('storage') }}/' + foto.ruta_archivo;
                    let tam = foto.tamanio ? (foto.tamanio / 1024).toFixed(1) + ' KB' : '';
                    agregarFotoGrid(idx, foto.id, url, tam);
                });
            }
        });
    }
...
// ESTADO GLOBAL
// ═══════════════════════════════════════════════════════
let siniestroId      = {{ isset($siniestro_edit) ? $siniestro_edit->id : 'null' }};
let elementoCount    = 0;
...
async function verificarDespacho(codigo, nombre, correo, dir, ciudad, telefono) {
  ...
  try {
    ...
    if (data.es_borrador && !siniestroId) {
       // Redirige transparentemente para cargar los datos del borrador sin preguntar
...
let empleadosAgregados = @json(isset($siniestro_edit) ? ($siniestro_edit->empleados_json ?? []) : []);
...
async function guardarOActualizarCabecera() {
  ...
  const datos = {
    ...
    titular_telefono:   $('#t-telefono').val(),
...
$('#t-nombre, #t-cedula, #t-cargo, #t-correo, #t-telefono').on('input', scheduleAutosave);
...
async function guardarBorradorYSalir() {
  ...
  if (sinGuardar > 0) {
    const res = await Swal.fire({
      ...
      html: `Tiene <strong>${sinGuardar}</strong> elemento(s) agregado(s) pero no guardado(s).<br>Si sale ahora, perderá esos elementos no guardados.<br><br>¿Desea salir de todos modos?`,
      icon: 'warning',
      showCancelButton: true,
  ...
  // Guardar la cabecera forzadamente
  await guardarOActualizarCabecera();

  Swal.fire({
     icon: 'success',
     title: 'Borrador Guardado',
     text: 'La información del siniestro se guardó como borrador exitosamente.',
     showConfirmButton: false,
     timer: 1500
  }).then(() => {
     window.location.href = "{{ route('encuesta.siniestro.index') }}";
  });
}
</script>
@endpush
