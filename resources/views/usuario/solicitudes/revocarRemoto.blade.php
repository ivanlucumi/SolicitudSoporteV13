    ...
    <div class="container-fluid">
        <dvi class="row">
         ...
         <div class="row ">
            ...
            <form action="{{ route('usuario.subir.revocar.trabajo.remoto',$solicitud->id,'enctype'=>&quot;multipart/form-data&quot;) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
...
@enderror
            ...
            <div class=" col-xs-12 col-md-12" style="display:back" id="id_formalizacion">
                <label for="archivo">Revocar Teletrabajo:</label>  <br>                          
                <input accept=".pdf" class="form-control-file form-group @error('revocar') is-invalid @enderror" id="formalizacion" type="file" name="revocar">
@error('revocar')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror
            </div>
            <div class="col-xs-12 form-group ">
                <label for="observaciones_revocado">Descripci&oacute;n : </label>
                <textarea class="form-control  @error('observaciones_revocado') is-invalid @enderror" id="demandante" autocomplete="off" placeholder="Describa Motivos para Revocar Teletrabajo" name="observaciones_revocado">{{ old('observaciones_revocado', $solicitud->observaciones_revocado ?? '') }}</textarea>
@error('observaciones_revocado')
    <div class="invalid-feedback">{{ $message }}</div>
@enderror
            </div>
