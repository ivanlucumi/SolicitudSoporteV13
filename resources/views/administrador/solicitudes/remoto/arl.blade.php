...
@section('cabecera', 'Solicitudes Teletrabajo Funcionarios')
@section('content') 
        ...
        <thead class="shadow" style="background-color: #004182; color: #fff;">
            ...
            @if($solicitudes != null)
             @foreach($solicitudes as $radicado)
                ...
                <tr @if($radicado->id_teletrabajo ==  auth()->user()->id)
                <?php echo 'style="background-color: #BCFFC5"'; ?>
                @endif
                @if(empty($radicado->id_teletrabajo))
                <?php echo 'style="background-color: #"'; ?>
                @endif
                @if($radicado->id_teletrabajo !=  auth()->user()->id)
                ...
                @endif>
                 ...
                 <td COLSPAN="3">
                     ...
                     @if( auth()->user()->tipo_rol == "ARL")  
                             <div class="col-xs-12 mt-3 mb-3" style="display:@if(!empty($radicado->concepto_arl))<?php echo "none"   ?> @endif">
                                 @if($radicado->id_teletrabajo ==  auth()->user()->id || empty($radicado->id_teletrabajo))
                                 <a href="{{ route('admin.solicitud.trabajo.remoto.concepto', $radicado->id) }}" class="btn btn-warning btn-xs btn-block" title="CONCEPTO ARL">CONCEPTO ARL</a>
                                 @else
                                 <button class="btn btn-warning btn-xs btn-block" disabled>EN TRÁMITE</button>
                                 @endif
                             </div>
                                 
                             @if($radicado->documento_concepto == null)
                               <a href="{{ route('admin.trabajo.remoto.nuevoconcepto', $radicado->id) }}" class="btn btn-warning btn-xs btn-block" title="SUBIR CONCEPTO ARL">SUBIR CONCEPTO ARL</a>
