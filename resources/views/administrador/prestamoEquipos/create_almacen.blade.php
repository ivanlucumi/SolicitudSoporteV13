...
<div class="container-fluid" style="background:#f8f9fc; min-height:80vh; padding:20px;">
  ...
  <form action="{{ route('almacen.prestamo.equipos.store') }}" method="POST" id="form-acta">
    @csrf
    ...
    <div class="panel panel-default" style="margin-bottom:16px;">
      ...
      <div class="panel-body">
        <div class="row">
          ...
          <div class="col-xs-12 col-sm-3">
            <div class="form-group">
              <label><i class="fa fa-building"></i> Despacho / Dependencia <span class="text-danger">*</span></label>
              <select class="form-control select2" name="codigo_despacho" id="codigo_despacho" style="width: 100%;" required>
                <option value="">-- Seleccione un Despacho --</option>
                @foreach($despachos as $d)
                  <option value="{{ $d->codigoDespacho }}" 
                          data-edificio="{{ $d->edificio }}" 
                          data-piso="{{ $d->piso }}"
                          {{ old('codigo_despacho') == $d->codigoDespacho ? 'selected' : '' }}>
                    {{ $d->nombreDespacho }}
                  </option>
                @endforeach
              </select>
                  ...
                  <tr>
                    <td>
                      <select name="empleados[0][elementos][0][elemento]"
                              class="form-control input-sm select-elemento select2" style="width: 100%;" required>
...
var OPCIONES_ELEMENTOS = [
  'Todo en uno', 'Teclado', 'Mouse', 'Escáner', 'Impresora', 'Portátil', 'Silla', 'Mesa', 'Escritorio', 'Diadema', 'Descansa pies'
];
...
function buscarTitular() {
  ...
  fetch("{{ route('almacen.prestamo.equipos.buscar_empleado') }}?cedula=" + cedula)
    .then(r => r.json())
    .then(data => {
      if (data.encontrado) {
...
function bindEventos() {
  ...
  if (typeof jQuery !== 'undefined') {
      jQuery('.select-elemento').off('change.misel').on('change.misel', function() {
...
function bindEventosElementos(bloque) {
  ...
  if (typeof jQuery !== 'undefined') {
      jQuery(bloque).find('.select-elemento').off('change.misel').on('change.misel', function() {
          validarDuplicadoElemento(bloque);
      });
  } else {
...
// Inicializar
bindEventos();
// Función auxiliar para inicializar Select2
function inicializarSelect2() {
    if (typeof jQuery !== 'undefined' && jQuery.fn.select2) {
        if (!jQuery('#codigo_despacho').hasClass('select2-hidden-accessible')) {
            jQuery('#codigo_despacho').select2({
        ...
        jQuery('.select-elemento:not(.select2-hidden-accessible)').select2({
...
if (typeof jQuery !== 'undefined') {
    jQuery(document).ready(function() {
        ...
        // El change de select2 no siempre dispara el change nativo en el código existente.
        // Lo disparamos explícitamente para el despacho
        jQuery('#codigo_despacho').on('change', function() {
            var selectEl = document.getElementById('codigo_despacho');
            if(!selectEl || selectEl.selectedIndex < 0) return;
            var option = selectEl.options[selectEl.selectedIndex];
            
            var edificio = option.getAttribute('data-edificio');
            var piso = option.getAttribute('data-piso');
            
            var edificioInput = document.getElementById('edificio');
            var pisoInput = document.getElementById('piso');
            
            if(edificioInput) {
                edificioInput.value = (edificio && edificio !== 'null' && edificio !== 'undefined') ? edificio : '';
            }
            if(pisoInput) {
                pisoInput.value = (piso && piso !== 'null' && piso !== 'undefined') ? piso : '';
            }
        });
    });
}
